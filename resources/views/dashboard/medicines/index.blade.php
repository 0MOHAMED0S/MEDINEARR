...
@section('content')
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mb-8">
            <div class="stat-card bg-white p-5 md:p-6 rounded-[2rem] border border-gray-100 shadow-sm flex items-center gap-4 md:gap-5 transition-all hover:shadow-md hover:border-primary/20 group">
                ...
                <div class="min-w-0 text-right">
                    <p class="text-[10px] md:text-sm text-gray-500 font-bold truncate mb-1">إجمالي الأدوية</p>
            ...
            <div class="stat-card bg-white p-5 md:p-6 rounded-[2rem] border border-gray-100 shadow-sm flex items-center gap-4 md:gap-5 transition-all hover:shadow-md hover:border-lime-200 group">
                <div class="w-12 h-12 md:w-16 md:h-16 bg-lime-50 text-accent rounded-xl md:rounded-2xl flex items-center justify-center text-xl md:text-3xl shrink-0 group-hover:scale-110 transition-transform">
                    <i class="fa-solid fa-check-circle"></i>
                </div>
                <div class="min-w-0 text-right">
                    <p class="text-[10px] md:text-sm text-gray-500 font-bold truncate mb-1">أدوية متاحة (نشطة)</p>
            ...
            <div class="stat-card bg-white p-5 md:p-6 rounded-[2rem] border border-gray-100 shadow-sm flex items-center gap-4 md:gap-5 transition-all hover:shadow-md hover:border-red-200 group">
                <div class="w-12 h-12 md:w-16 md:h-16 bg-red-50 text-red-500 rounded-xl md:rounded-2xl flex items-center justify-center text-xl md:text-3xl shrink-0 group-hover:scale-110 transition-transform">
                    <i class="fa-solid fa-ban"></i>
                </div>
                <div class="min-w-0 text-right">
                    <p class="text-[10px] md:text-sm text-gray-500 font-bold truncate mb-1">أدوية غير متاحة</p>
                            ...
                            <tr class="medicine-row hover:bg-slate-50/80 transition-all duration-300 group {{ $medicine->status == 0 ? 'opacity-60 grayscale-[20%]' : '' }}" data-status="{{ $medicine->status }}">
                                ...
                                <td class="p-4 md:p-5">
                                    <div class="flex flex-col">
                                        <span class="font-bold text-slate-800 text-sm md:text-base mb-0.5">{{ $medicine->name }}</span>
                                        <span class="text-[10px] md:text-xs text-gray-500 line-clamp-1 max-w-xs" title="{{ $medicine->description }}">{{ $medicine->description }}</span>
                                    </div>
                                </td>

                                <td class="p-4 md:p-5 text-center">
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-100 text-slate-600 text-[10px] md:text-[11px] font-bold border border-slate-200">
                                        <i class="fa-solid fa-layer-group text-[9px] md:text-[10px] text-primary"></i>
                                ...
                                <td class="p-4 md:p-5 text-center">
                                    <button type="button"
                                            ...
                                            class="relative inline-flex h-6 w-12 items-center rounded-full transition-colors duration-300 ease-in-out focus:outline-none {{ $medicine->status ? 'bg-emerald-500' : 'bg-gray-300' }}">
                                        <span id="toggle-knob-{{ $medicine->id }}"
                                              class="inline-block h-4 w-4 transform rounded-full bg-white transition duration-300 ease-in-out {{ $medicine->status ? '-translate-x-7' : '-translate-x-1' }}">
                                        </span>
                                    </button>
                                </td>
            ...
            <form id="editMedicineForm" action="{{ old('form_type') == 'edit' ? old('update_url') : '' }}" method="POST" enctype="multipart/form-data" class="flex flex-col flex-1 overflow-hidden" onsubmit="disableSubmitButton(this)">
                ...
                <div class="p-5 md:p-8 space-y-5 md:space-y-6 overflow-y-auto scrollbar-thin text-right">
                    ...
                    <div class="flex items-center justify-between bg-blue-50/50 p-4 md:p-5 rounded-xl md:rounded-2xl border border-blue-100">
                        <label class="text-xs md:text-sm font-black text-blue-800">حالة الدواء</label>
                        <label class="relative inline-flex items-center cursor-pointer group/switch">
    ...
    <div id="deleteMedicineModal" class="fixed inset-0 z-50 hidden flex-col items-center justify-center p-4 sm:p-0">
        ...
        <div class="relative bg-white rounded-[2.5rem] w-full max-w-sm shadow-2xl overflow-hidden animate-scale-up text-center p-6 md:p-10">
            ...
            <h3 class="text-xl md:text-2xl font-black text-slate-800 mb-2">حذف الدواء؟</h3>
            <p class="text-xs md:text-sm text-gray-500 font-medium mb-6 md:mb-8 leading-relaxed">سيتم حذف الدواء نهائياً من النظام. هل أنت متأكد؟</p>

            <form id="deleteMedicineForm" action="" method="POST" onsubmit="disableSubmitButton(this)">
                @csrf @method('DELETE')
                <div class="flex items-center justify-center gap-3">
                    <button type="button" onclick="toggleModal('deleteMedicineModal')" class="flex-1 px-4 md:px-6 py-3.5 md:py-4 rounded-xl md:rounded-2xl font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors text-xs md:text-sm">إلغاء الأمر</button>
    ...
    <script>
        ...
        function openDeleteModal(id) {
            document.getElementById('deleteMedicineForm').action = `/admin/medicines/${id}`;
        ...
        function openToggleConfirmModal(id, name, isCurrentlyActive) {
            ...
            if (isCurrentlyActive) {
                ...
                message.innerHTML = `هل أنت متأكد أنك تريد <strong>إيقاف</strong> إتاحة دواء "${name}"؟<br>لن يظهر هذا الدواء للمستخدمين.`;
                ...
                message.innerHTML = `هل أنت متأكد أنك تريد <strong>تفعيل</strong> إتاحة دواء "${name}"؟<br>سيظهر هذا الدواء بشكل طبيعي.`;
        ...
        function executeToggle() {
            ...
            fetch(`/admin/medicines/${currentToggleId}/toggle-status`, {
                method: 'PATCH',
                headers: {
