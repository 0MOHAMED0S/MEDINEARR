...
class CouponController extends Controller
{
    ...
    public function update(Request $request, $id) // <-- تم استخدام $id بدلاً من Coupon $coupon
    {
        $coupon = Coupon::findOrFail($id); // جلب الكوبون بشكل صريح ومضمون

        $request->validate([
            'title'          => 'required|string|max:100',
            // استخدام Rule::unique هو الطريقة الأكثر احترافية وأماناً لتجاهل الكود الحالي
            'code'           => [
                'required',
                'string',
                'max:50',
    ...
    public function destroy($id) // <-- تم استخدام $id
    {
    ...
    public function toggleStatus($id) // <-- تم استخدام $id
    {
        try {
            $coupon = Coupon::findOrFail($id);
